---
title: Technical Upskill Session - Config Sync Group Use Case Fix It Lab
...
complete. You will notice that the group will be out-of-sync and unable to resolve itself. Your objective is to find
out why this is happening and resolve the issue.
...
> Note the line **"unknown directive "api" in /etc/nginx/conf.d/demo.conf:23"**.
...
The NGINX OSS instances could have been added by mistake so we would confirm if this config sync group should have both
NGINX Plus and NGINX OSS.

### Solution 2

Lets say the different NGINX OSS and NGINX Plus is expected to belong in the same config sync group.
