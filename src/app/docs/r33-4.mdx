...
Let's now look at logs. There are two components where usage reporting logs may be of use when it comes to
troubleshooting.
...
changed the time on the NGINX Plus instance to about 100 days in the future cause issues when times are checked.
