...
license activation process does happen automatically when it is connected to the internet but for this use case, we must
manually handle the activation process. The steps here go through those manual steps.

This lab already has NIM installed with a license but it isn't activated. Let's look at the NIM UI to confirm it is
...
You now submitted the usage report to F5! You will need to do this at least every 180 days.
...
Let's now look at the logs. There are two components where usage reporting logs may be of use when it comes to
troubleshooting.
...
Any logs messages relating to submitting usage reports can be found in the **error_log** directive. Since I am using the
default values, it is located in `/var/log/nginx/error.log`.
...
tries to submit the report.

```log
2025/04/03 20:53:45 [error] 2234#2234: certificate verify error: (10:certificate has expired) for 3.135.72.139:443 during usage report
2025/04/03 20:53:45 [alert] 2234#2234: cannot send usage report; traffic processing will degrade in 59 days
```

When the usage report is successfully submitted, you will see **info** level log messages.
...
the log level to see them in the NIM logs. For the example here, I used the default log location, so it in `/etc/nms/nms.log`

If there are any issues, you will see **WARN** and **INFO** log messages coming from **DPM**. In my example here, I
changed the time on the NGINX Plus instance to about 100 days in the future to cause issues when the times are checked.
