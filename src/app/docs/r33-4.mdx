...
following **mgmt** section. We will be doing the following:
    - Set the **usage_report** endpoint to `nim-disconnected.local` which is our NIM instances
    - Set a quicker **interval** so we can see the frequency from the NGINX logs.
    - Disable **ssl_verify**
    ```shell
    mgmt {
      usage_report endpoint=nim-disconnected.local interval=10m;
      ssl_verify off;
    }
    ```
...
If there are any issues, you will see **WARN** and **INFO** log messages from **DPM**. In my example here, I
changed the time on the NGINX Plus instance to about 100 days in the future. This causes issues when the F5 Licensing
