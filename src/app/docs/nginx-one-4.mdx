---
title: Use Case 1 Interactive Lab
description: Managing NGINX Instances Interactive Lab
order: 4
---

## Use Case 1: Managing NGINX Instance Management Interactive lab

Now lets continue to the interactive lab where you will learn how to add NGINX instances to the NGINX One Console.
...
1. From the NGINX One Console, in the left menu under the "Manage" section, select **Data Plane Keys**.
...
1. Go to your UDF Deployment and connect to the "NGINX OSS" system using the Web Shell access method.

1. Because the hostname is used as the name of the instance in the NGINX One Console, you should change the hostname to
 **<GetVariable name="petname" />-ubuntu-oss**. Ensure that you are
...
the example command by selecting the copy button.
...
1. Explore the instance details.
    ...
    Package maintainers may ship NGINX with their own sets of defaults, which may or may not align with best
...
container image. The container image we will use here is **private-registry.nginx.com/nginx-plus/agent:r32-debian** which
...
addition to the originally provided command, we added two options that defines the container's `hostname` and `name`.
    ...
    -d private-registry.nginx.com/nginx-plus/agent:r32-debian
    ```
...
1. Return to your NGINX One Console.
...
from the NGINX container instance. The path to the NGINX agent logs are located at the same location as the virtual
machine instance, **/var/log/nginx-agent**. Now let's examine the logs by connecting to the container.

1. Go to your UDF Deployment and start a Web Shell session to **Lab Framework**.
...
### Explore NGINX Instance console
...
<Collapsible title="Instance Dashboard Details">
...
<Collapsible title="NGINX One Console Dashboard Details">
...
1. Check that the *stub_status* module is working by clicking the *Check* button below.
    ...
    > The above is equivalent to running `curl http://10.1.1.6/nginx_status` on the **Lab Framework** system where
    **10.1.1.6** corresponds to the NGINX OSS system.
...
1. Check that the *stub_status* module is working with **TLS enabled** by clicking the *Check* button below.
    ...
    > The above is equivalent to running `curl -k https://10.1.1.6/nginx_status` on the **Lab Framework** system where
    **10.1.1.6** corresponds to the NGINX OSS system.
...
You have completed the interactive lab! Now lets clean up before proceeding to the Fix It Lab!
...
1. Open up a Web Shell for **NGINX OSS** and run the command below to remove the NGINX agent.
...
1. Add a VM NGINX instance
1. Add a container NGINX instance
1. View NGINX Agent logs
1. Publish NGINX configuration changes from NGINX One Console to NGINX instance
1. View the instance dashboard that includes Certificates, CVEs, and Config Recommendations.
1. View the NGINX One Console dashboard for the entire NGINX fleet that includes Certificates, CVEs, Config
Recommendations, Availability, NGINX Versions, and more

## Up next

Let's now proceed to the [Fix It Lab](nginx-one-5.mdx).
