...
Now lets continue to the interactive lab where you will learn how to manage NGINX instances to NGINX One Console's
Config Sync Group.
...
Documentation for both these methods can also be found from the Config Sync Group from the NGINX One Console.
...
1. Open a Web Shell to your **NGINX Plus** component and run the command below to set a unique hostname for the NGINX
One Console.
...
1. Now return to the SSH window for your NGINX Plus instance and re-open the **demo.conf** file. The location block we
...
Lets continue to the [Fix It Lab](nginx-one-8.mdx).
