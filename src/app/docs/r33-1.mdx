...
to **product.connect.nginx.com**, option 2a is another option. If this design, your NGINX Plus instance managed by NIM is
...
the NGINX Plus usage report and submit it to the F5 Licensing Endpoint. The person will then download the license
...
When implementing **Option 2a** or **Option 2b**, you have to allow egress and ingress traffic from NIM to the **F5
Licensing Endpoint**.
...
This lab goes through the steps of setting up and manually submitting the usage report in an environment where your
NGINX Plus instance and NIM is disconnected from the internet.
...
1. Jumphost
1. (Connected) NGINX Plus Instance R33 (for fresh R33 install)
1. (Disconnected) NGINX Plus Instance R33 (for fresh R33 install)
1. (Connected) NGINX Instance Manager 2.18
1. (Disconnected) NGINX Instance Manager 2.18
1. NGINX Plus Instance R32 (for R33 upgrade)
