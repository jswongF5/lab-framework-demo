...
1. Open up a Web Shell to the **NGINX Instance Manager (Connected)** instance in UDF.
...
following **mgmt** section. We will be doing the following:
    ...
    ```shell
    mgmt {
      usage_report endpoint=nim-connected.local interval=1m;
      ssl_verify off;
    }
    ```

    > :warning: It is not advisable to disable `ssl_verify`.
...
> :warning: You may not be able to force a fail when the NGINX instance submits the report in this scenario where NGINX
is reporting to NIM. During my testing, I was changing the date to a future date. For example, I ran
...
the log level to see them in the NIM logs. For the example here, I used the default log location, so it in
`/etc/nms/nms.log`.
...
100 days in the future. This causes issues when the F5 Licensing Endpoint check timestamps.
