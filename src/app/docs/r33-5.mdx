...
```shell
root@ip-10-1-1-4:~$ tail -f /var/log/nginx/error.log

2024/11/08 18:29:45 [error] 19725#19725: invalid license token
```

There is not a valid license so **nginx** does not start. Go ahead and update `/etc/nginx/license.jwt` with your valid
JWT you got in the previous section.

Once that is completed, restart **nginx**.

```shell
root@ip-10-1-1-4:~$ systemctl restart nginx
...
And it finally starts! You can now proceed to configuring your NGINX Plus instance.
