---
title: Use Case 1 Fix It Lab
description: Managing NGINX Instances Fix It Lab
order: 5
---
...
Plus container. The person thinks the command they copied looks correct. But after waiting hours, the instance still is
not showing up on the NGINX One Console.
...
**agent.connect.nginx.com:443**.
    ...
    We now highly suspect the Data Plane Key. Let's try to re-create this instance but this time, let's use the one we
    created in this lab.
...
1. Great, so far the agent logs look okay. Now navigate back to NGINX One Console and look at the *Instances*. You
should now see your fixed instance here.

</Collapsible>

Great job! You now know the starting point when you need to troubleshoot your NGINX instance.
...
1. For the Docker containers running NGINX run the commands below to stop and remove the container on **Lab Framework**
...
1. View logs on NGINX instance when troubleshooting

The goal of this Fix It lab is to get you comfortable finding and looking at logs on the NGINX instance when there are
issues with this instance and NGINX One Console.

## Up next

You have now completed the labs for the *Managing NGINX Instances using NGINX One Console* use case.

Let's proceed to the next use case where we will go through [Config Sync Groups](nginx-one-6.mdx).
