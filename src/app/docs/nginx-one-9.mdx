---
title: Use Case 1 Demo Lab
description: Managing NGINX Instances Demo Lab
order: 9
---

## What is NGINX One?

The instructor will be the one going through the Demo Lab. The only ask for you is just to watch.

> :warning: NOTE: This is an dynamic website so there are components in this page where code is handled behind the
scenes. You will be going through the manual steps in the next interactive lab.
...
Now that we have a data plane key, we can attach our first NGINX instance to the NGINX One Console.
    ...
    > **Note:** The Demo Lab focuses more on the NGINX One Console and less on how to install NGINX Agent on your NGINX
    instance and connect the instance to NGINX One.
...
Now that we have set up our data plane key and deployed an NGINX instance we can take a look around the NGINX One Console:
...
<Collapsible title="Additional NGINX One Console Instance Details">
...
<Collapsible title="Additional NGINX One Console Dashboard Details">
...
If you would like more in-depth details of the NGINX configuration you see here, expand the following section below.

<Collapsible title="Additional NGINX Configuration Details">

#### NGINX Configuration

Let's take a look at what the **/etc/nginx/nginx.conf** configuration is doing:
...
After this demo, you should understand the NGINX One Console view when it manages NGINX instances. The views includes
a page showing NGINX metrics as well as a page allowing one to change and publish the NGINX instance configuration.
...
1. Click **Stop** on all the Docker instances displayed below:
    ...
    > **Note:** If some of the instance shows a green **Run** button instead of a red **Stop** button, you will need to refresh this page.
...
[Proceed to the lab](./nginx-one-4.mdx)
